...
graph TB
    subgraph "Overpass API"
        O1[Single large request]
        O2[Returns all data at once]
        O3[Geometry included]
        O4[Retry on 429/5xx]
        O5[Exponential backoff]
    end

    subgraph "Wikidata API"
        W1[Multiple batch requests]
        W2[50 IDs per batch]
        W3[Metadata only]
        W4[Graceful error handling]
        W5[Fixed 100ms delay]
    end
